...
def load_data(file_path):
    """Load data from the Excel file and return a DataFrame and its columns."""
    try:
...
def generate_charts(df, column_chart_pairs, config, output_folder):
    """Generate charts for the selected columns and yield the image paths."""
    figsize = (8, 6)
    for column, chart_type in column_chart_pairs:
        if column in df.columns:
            ...
            if chart_type == 'Pie Chart':
                ...
                # Scatter plots require two columns
...
def create_pdf_report(config, charts, output_folder):
    """Generate a PDF report using the provided charts."""
...
class ChartSelectionDialog(tk.Toplevel):
    """Dialog for selecting chart types for each selected column."""
    ...
    def __init__(self, parent, columns, callback):
        ...
        # Create the chart selection options
        frame = tk.Frame(self)
...
class App:
    def __init__(self, root):
        ...
        # Generate Report and Map Buttons
    ...
    def open_file(self):
        """Open an Excel file and load its columns."""
    ...
    def select_output_folder(self):
        """Select the output folder for saving the report."""
    ...
    def show_chart_selection_dialog(self):
        """Open the chart selection dialog after selecting columns."""
    ...
    def set_column_chart_pairs(self, column_chart_pairs):
        """Set the column-chart pairs and generate the report."""
        self.column_chart_pairs = column_chart_pairs
        self.generate_report()

    def generate_report(self):
        """Generate the PDF report with selected charts."""
    ...
    def generate_map(self):
        """Generate an interactive map based on selected coordinates."""
        if not self.output_folder:
            messagebox.showerror("Error", "Please select an output folder first.")
            return
    ...
    def get_selected_coordinate_column(self, prompt):
        """Prompt the user to select a coordinate column."""
